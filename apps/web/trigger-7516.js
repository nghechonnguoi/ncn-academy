...
async function run() {
  ...
  if (doc.error) {
    console.log("Not found as NCN-7516, trying 7516...");
    const res2 = await fetch(`https://firestore.googleapis.com/v1/projects/nghechonnguoi-f9eec/databases/(default)/documents/orders/7516`);
    const doc2 = await res2.json();
    if (doc2.error) {
        console.error("Order not found!");
        return;
    }
    processDoc(doc2, '7516');
  } else {
