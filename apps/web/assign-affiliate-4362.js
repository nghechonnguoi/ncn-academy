/**
 * Ghi nhận hoa hồng: Cập nhật referralCode cho order NCN-4362 → VAN418
...
async function main() {
  ...
  if (check.status !== 200) { console.error(`❌ Order ${ORDER_ID} không tìm thấy (HTTP ${check.status})`); console.error(JSON.stringify(check.body, null, 2)); process.exit(1); }
  ...
  } else { console.error(`❌ Lỗi HTTP ${result.status}:`); console.error(JSON.stringify(result.body, null, 2)); process.exit(1); }
}
