...
export function ExportReportButton({ assessment, avoidCareers = [], className }: ExportReportButtonProps) {
  ...
  const handleExport = async () => {
    ...
    const payload: Record<string, string> = {
      ...
      NGAY_SINH: profile.birthDate ?? "Chưa cung cấp",
      NGAYSINH:  profile.birthDate ?? "Chưa cung cấp",
      EMAIL:    profile.email     ?? "Không cung cấp",
      DIEN_THOAI: profile.phone   ?? "Chưa cung cấp",
