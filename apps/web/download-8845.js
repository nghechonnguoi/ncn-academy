...
async function run() {
  ...
  if (doc.error) {
    console.log(`Not found as ${orderId}, trying NCN-${orderId}...`);
    const res2 = await fetch(`https://firestore.googleapis.com/v1/projects/nghechonnguoi-f9eec/databases/(default)/documents/orders/NCN-${orderId}`);
    const doc2 = await res2.json();
    if (doc2.error) {
        console.error("Order not found!");
        return;
    }
    processDoc(doc2, `NCN-${orderId}`);
  } else {
...
async function processDoc(doc, id) {
  ...
  console.log(`Extracted payload for: ${payload.HOTEN || payload.HO_TEN} | Email: ${payload.EMAIL}`);
